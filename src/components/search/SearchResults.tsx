...
import { CalendarIcon, EyeIcon, Tag, TagIcon, UserIcon } from 'lucide-react'
...
const ErrorMessage = memo(({ error }: { error: string }) => (
  <div
    className="error-message text-center py-8"
    role="alert"
    aria-live="assertive"
  >
...
const SearchResults: React.FC<SearchResultsProps> = ({
  ...
  const searchResultItems = useMemo(() => {
    return results.map((result, index) => {
      ...
      const handleClick = createHandleClick(result)
      ...
      return (
        <div
          ...
          className={cn(
            ...
            isSelected
              ? 'search-result-selected search-result-focus'
              : 'hover:bg-muted/30',
            ...
            <CardHeader className="pb-3 p-4 sm:p-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                <div className="flex items-center gap-1.5">
                  <Avatar className="w-4 h-4 sm:w-5 sm:h-5">
                    <AvatarImage
                      ...
                      alt={`${result.author.name} 프로필 이미지`}
                    />
                    <AvatarFallback>
                      <UserIcon
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  />
                  <span
                    aria-label={`조회수 ${result.views.toLocaleString()}회`}
                  >
                ...
                {result.category && (
                  <Badge
                    ...
                    style={{
                      backgroundColor: result.category.color || undefined,
                    }}
                  >
            ...
            <CardContent className="pt-0 px-4 sm:px-6 pb-4 sm:pb-6">
              <p className="text-sm sm:text-base text-gray-700 dark:text-gray-300 line-clamp-3 mb-3 sm:mb-4 leading-relaxed">
                <HighlightedText
                  ...
                  query={searchQuery}
                  aria-label={`내용 미리보기: ${result.content.slice(
                    0,
                    150,
                  )}...`}
                />
              ...
              {result.tags && result.tags.length > 0 && (
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  ...
                  {result.tags.length > 3 && (
                    <Badge
                      variant="outline"
                      className="text-xs sm:text-sm text-gray-500 py-1 px-2"
                    >
                      +{result.tags.length - 3}
                    </Badge>
                  )}
  ...
  }, [results, searchQuery, selectedIndex, createHandleClick])
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      {/* 검색 결과 개수 표시 */}
      <div
        className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-3 sm:mb-4 px-1"
        aria-live="polite"
      >
